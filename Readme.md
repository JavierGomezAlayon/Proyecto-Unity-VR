...
- El personaje en los niveles se mueve de forma automática hacía adelante.  
- El jugador dispone de 2 pistolas (una en cada mano) con las que tendrá que matar a los enemigos apuntando con ellas.  
- Las pistolas no tienen munición infinita, se dispone de 10 balas en cada pistola que se podrán recargar si el jugador apunta hacía abajo y pulsa un botón.  
- El jugador puede utilizar un escudo si se mira la muñeca y dice la palabra ‘escudo’, así los enemigos no podrán hacerle daño.
...
- [ ] **Velocidad Constante**: El usuario se mueve a velocidad constante; se evitan aceleraciones y frenazos bruscos que el cuerpo no siente.
- [ ] **Control del Usuario**: El usuario tiene el control del movimiento (anticipación). Si es un movimiento forzado (ej. montaña rusa), el usuario inicia la acción, no arranca sola.
...
- [ ] **Transiciones de Luz Suaves**: Se evitan cambios bruscos de entornos oscuros a muy brillantes para no causar incomodidad visual.

#### 2. Interfaz de Usuario (UI) e Interacción
> Adaptación al "Canvas infinito" y limitaciones de interacción.

- [ ] **Inicio por Confirmación**: La experiencia comienza solo cuando el usuario confirma que está listo (click en pantalla inicial), nunca automáticamente al cargar.
- [ ] **Posición de UI**: Los controles aparecen dentro del campo de visión inicial y se reorientan si el usuario se mueve.
- [ ] **Uso de Retícula (Reticle)**: Se usa una retícula para ayudar a apuntar a objetivos pequeños o lejanos.
...
- [ ] **Separación de Elementos**: Los elementos interactivos están suficientemente separados para evitar selecciones erróneas.

#### 3. Inmersión y Presencia
> Coherencia entre expectativas y estímulos sensoriales.

- [ ] **Representación de Manos**: Se usan manos estilizadas (cartoon/robóticas) en lugar de realistas para evitar la disonancia con las manos reales.
- [ ] **Propiocepción**: No se representan brazos ni codos, solo las manos, para evitar romper la inmersión por fallos en la estimación de la postura.
...
- [ ] **Audio Espacial**: Se utiliza sonido 3D para que los objetos tengan una localización auditiva coherente en el espacio.
- [ ] **Latencia Baja**: La respuesta a las acciones es inmediata (idealmente < 20 ms).

#### 4. Ergonomía y Seguridad
> Respeto a las zonas de confort físico y visual.

- [ ] **Escala Correcta**: El tamaño de los objetos es coherente con la escala del mundo real y las expectativas del usuario.
...
- [ ] **Zona de Confort Vertical**: El contenido principal evita obligar al usuario a mirar hacia arriba o abajo más de 60° (evitar dolor de cuello).
- [ ] **Zona de Confort Horizontal**: El contenido principal se mantiene en la zona central (visión cómoda aprox. 30° - 55°).
- [ ] **Distancia de Visualización**: Los elementos de interés se sitúan en la zona confortable (evitar poner cosas muy cerca de los ojos o excesivamente lejos).